...
export async function getHomeFeed(): Promise<HomeFeedData> {
  const [gardenArticles, personalArticles] = await Promise.all([getAllArticles(), getAllPersonalArticles()]);
  ...
  const allItems = [...gardenItems, ...personalItems].sort((a, b) => toComparableTime(b.date) - toComparableTime(a.date));
